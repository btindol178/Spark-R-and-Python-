# https://www.youtube.com/watch?v=rJawNrD3xlU&t=1504s
...
spark_installed_versions() # what versions do you have installed
...
# to see which jobs are running and see which ones you want to kill 
...
# load a smaller data set for now
data(mtcars) # run and click for it to become dataset


#load mtcars into spark
cars <- copy_to(spark_conn,mtcars,"mtcars")

# search for all tables in the connection
src_tbls(spark_conn)


# make a tibble to make mtcars as a tibble
tbl(spark_conn,"mtcars")

cars

####################################
# Now we will use our own personal dataset with advertising
###################################

# Download a dataset or zip file
df <- spark_read_csv(spark_conn, name = "Advertising1",path = "C:/Users/blake/OneDrive/Desktop/Advertising1.csv",header = TRUE, delimiter = ",",infer_schema = TRUE)
